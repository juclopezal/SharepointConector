...
Multi-field AND-filtered search of SharePoint list items by list URL
(`POST /v1/sharepoint/list/items:search`): typed filter values with correct
OData literal translation, schema-validated filters and `select` projection
against the list's real columns, optional ordering, and bounded/truncated
